...
Start by creating a `docker-compose.yml` file with all the required configuration
...
$ chmod o-r /opt/docker/pihole/docker-compose.yml
```

Then modify the Pi-Hole's Dnsmasq configuration to force it to listen on all Docker interfaces and not only on eth0.

```console
$ mkdir -p /opt/docker/pihole/config/dnsmasq.d/
$ echo "interface=docker0" >> /opt/docker/pihole/config/dnsmasq.d/10-docker.conf
$ echo "interface=br-*" >> /opt/docker/pihole/config/dnsmasq.d/10-docker.conf
```

Finally, update the Dnsmasq's host configuration to listen on port 5353 instead 53. If at any point, we need to remove Pi-Hole, the only setting to revert is this `port` option.
...
Once started, You can access the Pi-Hole administration on [http://pisin.local/admin/](http://pisin.local/admin/)
...
It's really useful to resolve the names to identify quickly the clients so I ended up with a workaround. When a client join the network, it requests an IP to the DHCPv4 server and Dnsmasq stores the MAC address, the assigned IP and the hostname in a leases file. At the same time, the client creates several IPv6 addresses and send a Neighbor Solicitation message for the configuration. Using `tcpdump`, I monitor this kind of message and when a NS is received, I try to find a client hostname matching the MAC address in the Dnsmasq leases file. Then I update a hosts file with the IPv6 addresses and the guessed hostnames (I append `-6` suffix to the hostnames to avoid any collision with the original Dnsmasq leases file).

The script is embedded in a Docker container so it's easier to control and manage.
