...
Update the Dnsmasq's host configuration to listen on port 5353 instead 53. If at any point, we need to remove Pi-Hole, the only setting to revert is this `port` option.
...
It's really useful to resolve the names to identify quickly the clients so I ended up with a kind of hack. When a client join the network, it requests an IP to the DHCPv4 server and Dnsmasq stores the MAC address, the assigned IP and the hostname in a leases file. At the same time, the client creates several IPv6 addresses and send a Neighbor Solicitation message for the configuration. Using `tcpdump`, I monitor this kind of message and when a NS is received, I try to find a client hostname matching the MAC address in the Dnsmasq leases file. Then I update a hosts file with the IPv6 addresses and the guessed hostnames (I append `-6` suffix to the hostnames to avoid any collision with the original Dnsmasq leases file).

The script will run in a Docker container so it's easier to control and manage.

{: .box-note}
I published the docker image of `snifipv6` on Docker Hub so you can jump to the docker-compose.yml section if you just want to setup the container and use `gpailler/snifipv6:latest` image name.
